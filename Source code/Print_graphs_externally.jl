...
for d in 1:1
    ...
    global pcc_results=zeros(nper,nbuses);

        global pathopf = string(opffolder, "/", "load(MW).csv");
        global dfopf = CSV.read(pathopf, DataFrame);
        global pcc_results = Matrix(dfopf[2:end,2:end]); 
   
    global ee_results=zeros(nper,nbess);

    global pathopf = string(opffolder, "/", "bess energy (MWh).csv");
    global dfopf = CSV.read(pathopf, DataFrame);
    global ee_results = Matrix(dfopf[1:end,2:end]); 
 ...
 total_demand=[sum(pcc_results[t,n] for n in N) for t in NPER];
 min_y=round(minimum(total_demand)/1000-1,digits=0);
 max_y=round(maximum(total_demand)/1000+1,digits=0);
 load = bar(1:1:24, 
             total_demand/1000,
 ...
 sample_bess = bar(1:1:24, 
             [(pchh_results[:,sample_battery,d]) pdiss_results[:,sample_battery,d]],
             label=["Charging power" "Disharging power"], 
 ...
 global special_bar = DataFrame( 
        hour=1:1:8,
        charge=pchh_results[1:8,sample_battery,d],
        discharge=pdiss_results[1:8,sample_battery,d]);
        special_bar=stack(special_bar, [:charge, :discharge])
       rename!(special_bar, ["Hour", "Op", "Power"])
       
  special_graph=special_bar |> @vlplot(:bar, 
    column={"Hour", title="Hour (h)"}, 
    x={"Op", title=" "}, 
    y={"Power", title="Power (MW)"}, 
    color={"Op", legend=:false}, config={})
    save(string(opffolder,"/","operation of battery2.pdf"),   special_graph)
    # config={axis={labelfont="monospace"}})
 ...
 global genres_results_df.hour = genres_results_df.hour .-1;
