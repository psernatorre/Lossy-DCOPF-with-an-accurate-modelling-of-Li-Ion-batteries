...
for d in 1:ndays
    ...
    global objtradch_results[:,d] = Matrix(objtrad[1:end,2:end])[:,3].*(-1)./1000;  
end
...
for d in 1:ndays
    ...
    global objaccdis_results[:,d] = Matrix(objtrad[1:end,2:end])[:,2]./1000;  
    global objaccdch_results[:,d] = Matrix(objtrad[1:end,2:end])[:,3].*(-1)./1000;  
end
...
for d in 1:ndays
            ...
            fig = plot(range(1, nper, length=nper), 
                ...
                label=["Traditional BESS modeling" "Accurate BESS modeling"], 
                xticks=1:1:nper;
                xlabel="Hour(h)", 
                ylabel="kUSD",
                linewidth=2,
