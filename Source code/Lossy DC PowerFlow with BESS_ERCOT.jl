...
losses_modeling=parse(Int, config[13,2]);
stop_modeling=parse(Int, config[14,2]);
...
println(string("Formulation of ohmic losses: ", losses_modeling==0 ? "Using cosines" : "Using rF^2"))
println(string("Stopping criteria of the algorithm: ", stop_modeling==0 ? "Losses" : "Generation"))
...
for d in 1:1
    ...
    loadp_results[1:2,1]=["order" "bus_agg"];
        ...
        while error_losses[iter] >= tol_losses && iter<=iter_losses_max;
            ...
            #Calculate stopping criteria of the algorithm
            if stop_modeling==0
